...
class LeCartCustom(LeBasecart):
	...
	def display_import_source(self):
		...
		queries = {
			'taxes': {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM  stateTaxStates WHERE id > " + to_str(self._notice['process']['taxes']['id_src']),
			},
			# 'manufacturers': {
			# 	'type': 'select',
	...
	def convert_category_export(self, category, categories_ext):
		...
		# img = get_row_from_list_by_field(categories_ext['data']['categories_description'],'categories_id', category['categories_id'])
		img = category['categories_image']
		if img !='':
			category_data['thumb_image']['url'] = 'http://localhost:8080/customcart/images'
	...
	def convert_product_export(self, product, products_ext):
		...
		url_product_image = 'http://localhost:8080/customcart/images'
		...
		if product_description:
			for language_id in self._notice['src']['languages_select']:
				...
				product_data['name'] = product_description_lang['products_name']
				product_data['description'] = product_description_lang['products_description']
				product_data['short_description'] = product_description_lang['products_description']
	...
	def get_customers_ext_export(self, customers):
		...
		customers_ext_rel_queries = {
			...
			'states': {
				'type': 'select',
				'query': "SELECT * FROM  zones WHERE zone_code IN " + self.list_to_in_condition(zone_ids),
			}
		}
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		if address_books:
			for address_book in address_books:
				...
				country = get_row_from_list_by_field(customers_ext['data']['countries'], 'countries',address_book['entry_country_id'])
				if country:
					address_data['country']['id'] = country['id']
					address_data['country']['country_code2'] = country['countries_iso_code_2']
					address_data['country']['country_code3'] = country['countries_iso_code_3']
	...
	def get_orders_ext_export(self, orders):
		...
		bill_country = duplicate_field_value_from_list(orders['data'], 'billing_country')
		delivery_country = duplicate_field_value_from_list(orders['data'], 'delivery_country')
		country_ids = set(bill_country + delivery_country )
		#
		...
		orders_ext_queries = {
			...
			'countries': {
				'type': 'select',
				'query': "SELECT * FROM countries WHERE countries_iso_code_2 IN " + self.list_to_in_condition(country_ids)
			},

			# 'zones': {
			# 	'type': 'select',
			# 	'query': "SELECT * FROM  zones WHERE zone_code IN " + self.list_to_in_condition(state_ids)
			# }
		}
	...
	def convert_order_export(self, order, orders_ext):
		...
		# order_customer['username'] = order['customers_telephone']
		...
		customer_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_iso_code_2', order['billing_country'])
		if customer_country:
			customer_address['country']['id'] = customer_country['countries_id']
			customer_address['country']['code'] = customer_country['countries_iso_code_2']
		...
		order_billing['billing_name'] = order['billing_name']
		...
		# order_billing['state'] = order['billing_state']
		billing_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_iso_code_2', order['billing_country'])
		if billing_country:
		...
		order_delivery['delivery_name'] = order['delivery_name']
		...
		delivery_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_iso_code_2', order['delivery_country'])
		if delivery_country:
		...
		order_products = get_list_from_list_by_field(orders_ext['data']['orders_products'], 'order_id', order['orders_id'])
		...
		for order_product in order_products:
			...
			order_item['product']['id'] = order_product['orders_product_id']
	...
	def get_country_id(self, code, name):
		...
		if not countries or countries['result'] != 'success' or not countries['data']:
			return 0
		return countries['data'][0]['countries_id']
