...
class LeCartWordpress(LeBasecart):
	...
	def make_woocommerce_image_path(self, image, image_type = 'product'):
		...
		return 'wp-content/uploads/' + get_current_time('%Y/%m') + '/' + image_name
	...
	def wp_image(self, path, image_details = None, label = '', convert = dict):
		...
		image_query = self.create_insert_query_connector('wp_posts', post_data)
		...
		image_meta_query = self.create_insert_query_connector('wp_postmeta', meta)
		...
		attachment_metadata_query = self.create_insert_query_connector('wp_postmeta', attachment_metadata)
		...
		attachment_image_alt_query = self.create_insert_query_connector('wp_postmeta', attachment_image_alt)
