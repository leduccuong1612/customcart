...
class LeCartWoocommerce(LeCartWordpress):
	...
	def clear_target_manufacturers(self):
		...
		if self._notice['target']['support']['yoast_seo']:
			...
			if options_data and options_data['data']:
				...
				update_query = self.create_update_query_connector('wp_options', data_set, where)
	...
	def manufacturer_import(self, convert, manufacturer, manufacturers_ext):
		...
		if convert['thumb_image']['url'] or convert['thumb_image']['path']:
			...
			image_import_path = self.uploadImageConnector(image_process, self.add_prefix_path(self.make_woocommerce_image_path(image_process['path'], self.TYPE_CATEGORY), self._notice['target']['config']['image_manufacturer'].rstrip('/')))
		...
		if self.is_wpml() or self._notice['target']['support']['yoast_seo']:
			...
			if options_data and options_data['data']:
				...
				self.import_data_connector(self.create_update_query_connector('wp_options', data_set, where), 'manufacturer')
			else:
				new_option_data = {
					'option_name': 'wpseo_taxonomy_meta',
	...
	def category_import(self, convert, category, categories_ext):
		...
		categories_term_query = self.create_insert_query_connector('wp_terms', categories_term)
		term_id = self.import_data_connector(categories_term_query, 'categories')
		if not term_id:
		...
		categories_taxonomy_query = self.create_insert_query_connector('wp_term_taxonomy', categories_taxonomy)
		categories_taxonomy_import = self.import_category_data_connector(categories_taxonomy_query, True, convert['id'])
		if not categories_taxonomy_import:
			return response_warning('manufacturer ' + to_str(convert['id']) + ' import false.')
		self.insert_map(self.TYPE_CATEGORY, convert['id'], categories_taxonomy_import, convert['code'])
		thumbnail_id = False
		cate_image = ''
		if convert['thumb_image']['url'] or convert['thumb_image']['path']:
			image_process = self.process_image_before_import(convert['thumb_image']['url'], convert['thumb_image']['path'])
			image_import_path = self.uploadImageConnector(image_process, self.add_prefix_path(self.make_woocommerce_image_path(image_process['path'], self.TYPE_CATEGORY), self._notice['target']['config']['image_category'].rstrip('/')))
			if image_import_path:
				cate_image = self.remove_prefix_path(image_import_path, self._notice['target']['config']['image_category'])
				image_details = self.get_sizes(image_process['url'])
				thumbnail_id = self.wp_image(cate_image, image_details)
		if thumbnail_id:
			...
			meta_query = self.create_insert_query_connector('wp_termmeta', meta_insert)
			self.import_data_connector(meta_query, 'categories')
		else:
			meta_insert = {
				'term_id': term_id,
				'meta_key': 'brand_image_url',
			...
			meta_query = self.create_insert_query_connector('wp_termmeta', meta_insert)
			self.import_data_connector(meta_query, 'categories')
		return response_success(categories_taxonomy_import)
	...
	def update_latest_data_product(self, product_id, convert, product, products_ext):
		...
		# begin
		product_query = self.create_update_query_connector("wp_posts", {'ID': product_id, 'post_title': convert['name']}, {'ID': product_id})
		...
		for meta_key, meta_value in product_meta.items():
			...
			if meta_key == '_sale_price_dates_from' or meta_key == '_sale_price_dates_to':
				...
				if (not check_meta_key) or check_meta_key['result'] != 'success' or (not check_meta_key['data']) or (to_len(check_meta_key['data']) == 0):
					...
					meta_price_query = self.create_insert_query_connector("wp_postmeta", sale_price_data)
					all_query.append(meta_price_query)
			meta_query = self.create_update_query_connector("wp_postmeta", meta_insert, {'post_id': product_id, 'meta_key': meta_key})
		...
		if children_list and to_len(children_list) <= self.VARIANT_LIMIT:
			for key_child, product_child in enumerate(children_list):
				...
				for meta_key, meta_value in children_meta.items():
					...
					meta_query_child = self.create_update_query_connector('wp_postmeta', meta_insert_child, {'post_id': children_id, 'meta_key': meta_key})
	...
	def update_product_after_demo(self, product_id, convert, product, products_ext):
		...
		if convert['tax']['id'] or convert['tax']['code']:
			...
			if tax_class:
				...
				all_queries.append(self.create_update_query_connector('wp_postmeta', meta_insert, where_meta))
	...
	def product_import(self, convert, product, products_ext):
		...
		product_query = self.create_insert_query_connector('wp_posts', product_value)
		...
		product_meta_lookup_query = self.create_insert_query_connector('wp_wc_product_meta_lookup', product_meta_lookup)
		...
		for meta_key, meta_value in product_meta.items():
			...
			if meta_key == '_sale_price_dates_from' or meta_key == '_sale_price_dates_to':
				...
				check_meta_key = self.select_data_connector(query_meta_key, 'wp_postmeta')
				if (not check_meta_key) or check_meta_key['result'] != 'success' or (not check_meta_key['data']) or (to_len(check_meta_key['data']) == 0):
					sale_price_data = {
						'post_id': product_id,
						'meta_key': meta_key,
					...
					meta_price_query = self.create_insert_query_connector("wp_postmeta", sale_price_data)
					self.import_product_data_connector(meta_price_query, True, convert['id'])
			meta_query = self.create_insert_query_connector("wp_postmeta", meta_insert, {'post_id': product_id, 'meta_key': meta_key})
			self.import_product_data_connector(meta_query, True, convert['id'])
		# language_code = convert.get('language_code')
		# for category in convert['categories']:
		# 	category_id = self.get_map_field_by_src(self.TYPE_CATEGORY, category['id'], category['code'], language_code)
		...
		# 			'term_order': 0
		# 		}
		# 	term_relationship_query = self.create_insert_query_connector('wp_term_relationships', category_product_data)
		# 	self.import_product_data_connector(term_relationship_query, True, convert['id'])
		category_desc = list()
		for value in convert['categories']:
			...
			for category_id in category_ids:
				...
				# 		'meta_value': thumbnail_id
				# 	}
				# 	meta_query = self.create_insert_query_connector('wp_postmeta', meta_insert)
				# 	self.import_data_connector(meta_query, 'products')
				# 	meta_insert = {
				# 		'post_id': product_id,
				# 		'meta_key': 'brand_image_url',
				# 		'meta_value': self._notice['target']['cart_url'].rstrip('/') + '/wp-content/uploads/' + product_image.lstrip('/')
				# 	}
				# 	meta_query = self.create_insert_query_connector('wp_postmeta', meta_insert)
				# 	self.import_data_connector(meta_query, 'products')
			product_image = ""
	...
	def customer_import(self, convert, customer, customers_ext):
		for a in convert['address']:
			customer_values = {
				...
				'email': convert['email'],
				'date_last_active': convert['created_at'],
			...
			customer_query = self.create_insert_query_connector('wp_wc_customer_lookup', customer_values)
			customer_id = self.import_data_connector(customer_query,'customers', convert['id'])
	...
	def order_import(self, convert, order, orders_ext):
		...
		order_query = self.create_insert_query_connector('wp_posts', order_value)
		# order_id = self.import_data_connector(order_query,'orders')
		...
		order_stats_query = self.create_insert_query_connector('wp_wc_order_stats', order_stats_value)
		self.import_order_data_connector(order_stats_query, True, convert['id'])
		# for id, name in order.items():
		# 	order_product_lookup_value = {
		# 		'order_item_id': id,
		# 		'order_id': order_id
		# 	}
		# 	order_product_lookup_query = self.create_insert_query_connector('wp_wc_order_product_lookup', order_product_lookup_value)
		# 	self.import_data_connector(order_product_lookup_query, 'orders')
		# 	order_item_value = {
		# 		'order_item_id': id,
		...
		# 		'order_id': order_id
		# 	}
		# 	order_item_import_query = self.create_insert_query_connector('wp_woocommerce_order_items', order_item_value)
		# 	self.import_data_connector(order_item_import_query, 'orders')
		order_items = convert['items']
		for item in order_items:
			order_item_data = {
				'order_item_name': item['product']['name'],
			...
			order_item_query = self.create_insert_query_connector("wp_woocommerce_order_items", order_item_data)
			# order_item_id = self.import_data_connector(order_item_query, 'orders')
			...
			for meta_key, meta_value in order_item_meta.items():
				...
				meta_query = self.create_insert_query_connector("wp_woocommerce_order_itemmeta", meta_insert)
				# self.import_data_connector(meta_query, 'orders')
				order_id = self.import_order_data_connector(meta_query, True, convert['id'])
			for option in item['options']:
				meta_insert = {
					'order_item_id': order_item_id,
					'meta_key': option['option_name'],
	...
	def page_import(self, convert, page, pages_ext):
		...
		page_query = self.create_insert_query_connector('wp_posts', data)
	...
	def coupon_import(self, convert, coupon, coupons_ext):
		...
		coupon_query = self.create_insert_query_connector('wp_posts', coupon_data)
	...
	def after_coupon_import(self, coupon_id, convert, coupon, coupons_ext):
		...
		for meta_key, meta_value in coupon_meta.items():
			...
			meta_query = self.create_insert_query_connector("wp_postmeta", meta_insert)
			all_queries.append(meta_query)
		all_queries.append(self.create_update_query_connector('wp_posts', {'guid': self._notice['target']['cart_url'] + "/?post_type=shop_coupon&#038;p=" + to_str(coupon_id)}, {'ID': coupon_id}))
	...
	def display_finish_target(self):
		...
		option_query = self.create_insert_query_connector('wp_options', option_data)
	...
	def get_woo_attribute_value(self, attribute_value, pro_attr_code, language_code = None, attribute_data = None, desc = ''):
		...
		if not opt_value_id:
			...
			if not opt_value_id:
				...
				term_id = self.import_product_data_connector(self.create_insert_query_connector('wp_terms', value_term), 'products')
				value_term_taxonomy = {
					'term_id': term_id,
				...
				opt_value_id = self.import_product_data_connector(self.create_insert_query_connector('wp_term_taxonomy', value_term_taxonomy), 'products')
			if opt_value_id:
	...
	def check_exist_code_product(self, code_product):
		check = self.select_data_connector(self.create_select_query_connector('wp_posts', {'posttype'}))
